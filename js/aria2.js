if (typeof ARIA2=="undefined"||!ARIA2) var ARIA2=(function(){
  ...
  return {
    init: function(path, onready) {
      ...
      $("#add-task-option-wrap").empty().append(A2WEB.tpl.add_task_option({}));
      $("#aria2-gsetting").empty().append(A2WEB.tpl.aria2_global_setting({}));
    ...
    tell_active: function(keys) {
      ...
      ARIA2.request("tellActive", keys,
        function(result) {
          ...
          $("#active-tasks-table").empty().append(A2WEB.tpl.active_task({"tasks": result}));
    ...
    tell_waiting: function(keys) {
      ...
      ARIA2.request("tellWaiting", params,
        function(result) {
          ...
          result = ARIA2.status_fix(result.result);
          $("#waiting-tasks-table").empty().append(A2WEB.tpl.other_task({"tasks": result}));
          $.each(result, function(n, e) {
            $("#task-gid-"+e.gid).data("raw", e);
          });
          ...
          if (finished_tasks_list === undefined) {
            ...
            $.each(result, function(i, e) {
              ...
              if (ARIA2.finish_notification) {
                A2WEB.notification("下载完成", e.title);
              }
              finished_tasks_list.push(e.gid);
            });
          }

          $("#stopped-tasks-table").empty().append(A2WEB.tpl.other_task({"tasks": result.reverse()}));
          $.each(result, function(n, e) {
            $("#task-gid-"+e.gid).data("raw", e);
          });
    ...
    get_options: function(gid) {
      ARIA2.request("getOption", [gid],
        function(result) {
          ...
          $("#ib-options").empty().append(A2WEB.tpl.ib_options(result.result));
    ...
    get_peers: function(gid) {
      ARIA2.request("getPeers", [gid],
        function(result) {
          ...
          $("#ib-peers").empty().append(A2WEB.tpl.ib_peers(result.result));
        }
      );
    },
    ...
    get_global_option: function() {
      ARIA2.request("getGlobalOption", [],
        function(result) {
          ...
          $("#aria2-gsetting").empty().append(A2WEB.tpl.aria2_global_setting(result));
        }
      );
    },
    ...
    init_add_task_option: function() {
      ARIA2.request("getGlobalOption", [],
        function(result) {
          ...
          $("#add-task-option-wrap").empty().append(A2WEB.tpl.add_task_option(result));
        }
      );
    },
    ...
    global_stat: function() {
      ARIA2.request("getGlobalStat", [],
        function(result) {
          ...
          $("#global-speed").empty().append(A2WEB.tpl.global_speed(result));
          var title = "↓"+A2WEB.tpl.view.format_size_0()(result.downloadSpeed);
          if (result.uploadSpeed > 0)
            title += " ↑"+A2WEB.tpl.view.format_size_0()(result.uploadSpeed);
          title += " - 中國遠徵";
    ...
    get_status: function(gid) {
      ARIA2.request("tellStatus", [gid],
        function(result) {
          ...
          $("#ib-status").empty().append(A2WEB.tpl.ib_status(result));
          $("#ib-files .file-list").empty().append(A2WEB.tpl.files_tree(result.files));
          if ($("#task-gid-"+gid).attr("data-status") == "active")
            $("#ib-file-save").hide();
          if (result.bittorrent) {
            $("#ib-peers-a").show();
